...
class ProjectTags(models.Model):
    ...
    hr_employee_id = fields.Many2one(string="Employee", help="HR employee id", comodel_name="hr.employee")
    tag_id = fields.Many2one(string="Tag", help="Tag name of the project", comodel_name="project.tags")
    ...
    def _compute_tag_frequency(self):
        ...
        """
        for tag_id in self:
            tag_id.tag_frequency = self.env['project.task'].search_count([('user_id', '=', self._context.get('usr_id')), ('tag_ids', 'in', tag_id.id)])
    ...
    def _compute_fetch_users_of_tag(self):
        ...
        """
        for tag in self:
            tag.user_ids = list(set(map(lambda task: task.user_id.id, self.env['project.task'].search([('tag_ids', 'in', tag.id)]))))
